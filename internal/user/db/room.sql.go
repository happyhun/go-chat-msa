...
const listJoinedRoomIDsForUpdate = `-- name: ListJoinedRoomIDsForUpdate :many
SELECT rm.room_id
FROM room_members rm
JOIN rooms r ON r.id = rm.room_id
...
func (q *Queries) ListJoinedRoomIDsForUpdate(ctx context.Context, userID pgtype.UUID) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, listJoinedRoomIDsForUpdate, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
